...
def discover_all(root: str | Path) -> list[str]:
    ...
    if (root / "Cargo.toml").is_file() or any(root.glob("*.rs")):
        if "rust" not in out:
            out.append("rust")
    if (root / "compile_commands.json").is_file() or (root / "CMakeLists.txt").is_file() or any(
        root.glob("*.cc")
    ) or any(root.glob("*.cpp")) or any(root.glob("*.h")):
        if "cpp" not in out:
            out.append("cpp")
    return out
...
def build_index(root: str | Path) -> dict:
    """Incremental: reuse records whose sha256 still matches."""
    root = Path(root)
    ...
    data = {
        ...
        "stats": {"scanned": scanned, "reused": reused, "files": len(files)},
    }
...
def impact(root: str | Path, query: str, *, use_index: bool = True) -> dict:
    ...
    if idx.get("files"):
        ...
        for rec in idx["files"].values():
            ...
            hits += n
        return {"query": query, "hits": hits, "files": files_out, "source": source}
    ...
    for p in _iter_source(root, langs or None):
        ...
        hits += n
    return {"query": query, "hits": hits, "files": files_out, "source": "scan"}
...
def impact_from_scip(root: str | Path, query: str) -> dict:
    ...
    for occ in idx.get("scip") or []:
        ...
        if needle == sym or needle in sym.split(" ") or sym.endswith(needle) or needle in sym:
            p = occ.get("path") or ""
            files[p] = files.get(p, 0) + 1
    out = [{"path": p, "hits": n, "kind": "scip"} for p, n in files.items() if p]
    hits = sum(f["hits"] for f in out)
    return {"query": query, "hits": hits, "files": out, "source": "scip"} if hits else {"query": query, "hits": 0, "files": [], "source": "scip"}
